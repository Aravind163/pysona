...
const PLAN_PACKAGES = {
  standard_500:  { credits: 500,  rupees: 499,  planUpgrade: 'STANDARD', label: 'Standard Plan – 500 Credits' },
  premium_1500:  { credits: 1500, rupees: 999,  planUpgrade: 'PREMIUM',  label: 'Premium Plan – 1500 Credits' },
  ...
  // Premium top-ups at better rate (4 credits per rupee vs 3 for standard)
  premium_topup_500:  { credits: 500,  rupees: 125, planUpgrade: null, label: '500 Credits Top-Up (Premium rate)' },
  premium_topup_1000: { credits: 1000, rupees: 250, planUpgrade: null, label: '1000 Credits Top-Up (Premium rate)' },
};
...
router.post('/create-order', async (req, res) => {
  try {
    ...
    const orderPayload = {
      amount: amountPaise,
      currency: 'INR',
      receipt: `ord_${req.user._id.toString().slice(-8)}_${Date.now().toString().slice(-8)}`,
...
router.post('/verify', async (req, res) => {
  try {
    ...
    if (pkg.planUpgrade) updateFields.$set = { plan: pkg.planUpgrade, isApproved: true };
