...
const { authMiddleware, ADMIN_EMAILS } = require('../middleware/auth');
...
// Helper: check if email should be admin
...
router.post('/register', async (req, res) => {
  try {
    ...
    // Admins and paid users are always approved; free users wait for approval
    user.isApproved = isAdmin;
    user.lastActiveAt = new Date();
    await user.save();

    console.log(`[AUTH] New account: ${email} | role=${user.role} | isAdmin=${isAdmin}`);
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Auto-upgrade to admin if email is in admin list (handles ADMIN_EMAIL_2 late additions)
    ...
    if (shouldBeAdmin && user.role !== 'admin') {
      ...
      changed = true;
      console.log(`[AUTH] Auto-upgraded ${email} to admin`);
    }

    user.lastActiveAt = new Date();
    if (changed || true) await user.save(); // always save lastActiveAt
