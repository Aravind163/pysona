...
export const generateAIResponse = async (
  ...
  // Enforce minimum gap between requests to stay within free tier rate limit
  const now = Date.now();
  ...
  try {
    ...
    if (status === 502) throw new Error(msg || 'AI service error. Check your Gemini API key in backend .env');
