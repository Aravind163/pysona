...
const App = () => {
  ...
  const [isInitializing, setIsInitializing] = useState(true); // ← NEW: prevents flash to login
  ...
  const handleAuthSuccess = useCallback(async (newToken: string) => {
    setToken(newToken);
    localStorage.setItem(TOKEN_KEY, newToken); // ← persist token
    try {
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      handleAuthSuccess(savedToken).finally(() => setIsInitializing(false));
    } else {
  ...
  const logout = useCallback(() => {
    setToken(null);
    localStorage.removeItem(TOKEN_KEY); // ← clear persisted token
    setUser(null);
    disconnectSocket();
  ...
  // Show nothing while restoring session (prevents flash to login on refresh)
  if (isInitializing) {
    return (
